...
if(isset($_POST['submission']) && $_POST['submission'] === "true"){ // the form has been submitted, process it
    ...
    if(validateDate($lastSeen)){
        ...
        if($time > time()){
            $errors['lastSeen'] = "Woah there time traveller! Found On Date must be in the past!";
        }
    } else{
        $errors['lastSeen'] = "Found On DAte must be in the format dd/mm/yyyy HH:MM (eg. " . date("d/m/Y H:i") . ")";
    }
    ...
    if(count($errors) > 0){
        ...
        $insertSQL = "INSERT INTO cms_lostfounds (CharityID, CreatorID, CreatedOn, EditorID, EditedOn, LastSeen, Name, Description, Contact, Number1, Email, Details, isLost)"
                ...
                . "{$_SESSION['userID']}, NOW(), {$_SESSION['userID']}, NOW(), '{$petDetails['LastSeen']}', '{$petDetails['Name']}', '{$petDetails['Description']}', '{$petDetails['ContactName']}', '{$petDetails['Number']}', '{$petDetails['Email']}', '{$petDetails['Details']}', 0);";
...
<!-- Begin page content -->
      <div class="container">
        <div class="page-header">
          <h1>Submit Found Animal</h1>
        </div>
          <div class="col-md-9">
        <p class="lead">The animal you found has been added to our database.</p>
        <hr />
    ...
    ?>
      ...
      <div class="container">
        <div class="page-header">
          ...
          <div class="col-md-9">
        <p class="lead">You can use the form below to list the animal that you have found</p>
        <hr />
        <?php
        if(count($errors) > 0){
        ...
        <form method="post" enctype="multipart/form-data">
              ...
              <label for="lastSeen">Found On</label>
              <?php
