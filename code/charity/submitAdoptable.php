...
if(isset($_POST['submission']) && $_POST['submission'] === "true"){ // the form has been submitted, process it
    ...
    if(count($errors) > 0){
        ...
        $insertSQL = "INSERT INTO cms_pets (CharityID, CreatorID, CreatedOn, Name, Description, Contact, Number1, Email, Location, isAdoptable)"
                . "VALUES((SELECT CharityID FROM cms_charities WHERE DomainName = '{$_SESSION['lastDomain']}'),"
                . "{$_SESSION['userID']}, NOW(), '{$petDetails['Name']}', '{$petDetails['Description']}', '{$petDetails['ContactName']}', '{$petDetails['Number']}', '{$petDetails['Email']}', '{$petDetails['Location']}', 1);";
    ...
    ?>
      ...
      <div class="container">
        <div class="page-header">
          <h1>Submit Adoptable Pet</h1>
        </div>
          <div class="col-md-9">
        <p class="lead">You can use the form below to create a listing for an animal that can be adopted</p>
        <hr />
        <?php
        if(count($errors) > 0){
